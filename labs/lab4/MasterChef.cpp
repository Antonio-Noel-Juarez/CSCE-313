...
static void timerHandler( int sig, siginfo_t *si, void *uc )
{
    ...
    cout << "Completed Step: " << comp_item->id << " - " <<
        comp_item->description << endl;
	/* TODO This Section - 2 */
...
void RemoveDepHandler(int sig) {
    ...
    for (int i = 0; i < completeCount; i++) {
        int id = (*completedSteps)[i];
        recipeSteps->RemoveDependency(id);   // drop id as dependency everywhere
    }
	/* End Section - 3 */
}
...
int main(int argc, char **argv)
{
    ...
    sigaction(SIGRTMIN, &sa, NULL);       // Handle dependency removal
    sigaction(SIGUSR1, &sa, NULL);
    sa.sa_handler = RemoveDepHandler;

	// Until all steps have been completed, check if steps are ready to be run and create a timer for them if so
    while (recipeSteps->Count() > 0) {
        for (Step* next : recipeSteps->GetReadySteps()) {
            if (next != nullptr && !next->running) {
                next->running = true;
                makeTimer(next, next->duration);
            }
        }
